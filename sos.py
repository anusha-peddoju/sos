...
if((row!=1 and col!=1) and ((row<3 and col>=3 ) or (col<3 and row>=3)) or (row>=3 and col>=3)):	
	...
	while(cv<(row*col)):
		...
		if(a=="s" or a=="o" or a=="S" or a=="O"):
			...
			if(b in l):
				if(board[b]!="s" and board[b]!="o" and board[b]!="S" and board[b]!="O"):
					board[b]=a														#entering input to their places according to input
					cv+=1
					prin(board)														#function call
					print("\n")
				else:
					print("ENTERED BLOCK IS ALREADY FILLED TRY TO ENTER NEW ONE")
					check=1
			else:
				print("PLEASE ENTER CORRECT BLOCK")
				check1=1
		else:
		...
		if(cv>2 and check==0 and check1==0 and check2==0):
			...
			while(k4<row):				#for knowing whether total boxex are filled or not
				...
				if(count==(row*col)):			
					print("\n\t*** GAME IS OVER ***\n")
					print("the final scores of two players are:\n")
					cv=(row*col)
				k5=0
				k4=k4+1
			...
			if(cv==(row*col)):							#giving comment to their scores
				if(player1_score>player2_score):
					...
					print("congratulations to u",player1)
				elif(player2_score>player1_score):
					print(player2,"is the winner")
					print("congratulations to u",player2)
				elif(player1_score==0 and player2_score==0):
					print("no one won the game")		
				elif(player1_score==player2_score):
					print(player1_score,player2_score,"scores are equal")			
	...
	if(row==1 and col==1):
		print("SOS GAME IS NOT POSSIBLE WITH",   row  ,"ROW AND",   col   ,"COLUMN")
	elif(row==1 and col!=1):
		print("SOS GAME IS NOT POSSIBLE WITH",   row  ,"ROW AND",   col   ,"COLUMNS")
	elif(row!=1 and col==1):
		print("SOS GAME IS NOT POSSIBLE WITH",   row  ,"ROWS AND",   col   ,"COLUMN")
	else:
		print("SOS GAME IS NOT POSSIBLE WITH",   row  ,"ROWS AND",   col   ,"COLUMNS")
